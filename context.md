...
6. **Cuellos de botella de Ancho de Banda USB (Cámaras):** OpenCV por defecto intenta capturar video en formato crudo (YUYV). Para altas resoluciones (como 1080p), esto excede frecuentemente el ancho de banda físico de conexiones USB y causa que el driver (V4L2) aplique silenciosamente un *fallback* a resoluciones menores (ej. 640x360). Esto se resolvió forzando compresión por hardware inyectando `CAP_PROP_FOURCC = MJPG` en `main.cpp` justo antes de definir las dimensiones de la captura.
...
* **Inferencia Nativa C++ (Completada):** Se desarrollaron los ejecutables base utilizando la API `ov::Core`: `cbn_test_inference` (con GUI experimental), `cbn_inference_node` (nodo headless base) y `main` (nodo final de producción). Se reescribió el motor interno en `cbn_detector_inference.cpp` para soportar específicamente la arquitectura **YOLO26** (forma de tensor `[1, 300, 6]` con NMS incrustado), resolviendo problemas de memoria (*zero-copy*) y *offsets* en las coordenadas de *bounding boxes*.
* **Ejecutable Principal (`main.cpp`):** Coordina la lectura de cámara, aplicación de filtros visuales desde `config_cam0.yaml`, inferencia YOLO26 y la lógica de validación de casillero (devolviendo "PASA/RECHAZADO" cuando detecta las 12 posiciones con `cbn_detector.hpp`).
* **Rutas Dinámicas (C++)**: Se implementó un esquema de rutas robusto mediante `getWorkspacePath()` (vía `/proc/self/exe`), integrando la carga de etiquetas desde `ws_cpp/config/labels.yaml`, modelos e imágenes.
...
* **Integración de Hardware de Salida (GPIO):** Se desarrolló la librería `ws_cpp/include/cbn_io.h` para el control directo de los pines del Super I/O (IT8786E) mediante la utilidad nativa de Linux `gpioset`. Soporta operaciones estáticas y pulsos asíncronos (`std::thread::detach`) que no penalizan los FPS de la red neuronal. Se modificó el `Dockerfile` para asegurar que el paquete `gpiod` esté disponible en el entorno de contenedores y se redactó una guía de uso en `ws_cpp/README_GPIO.md`.
* **Conexión Inferencia-Física:** El archivo `ws_cpp/src/main.cpp` se enlazó a `Threads::Threads` en CMake y ahora orquesta la lógica de descarte final. Cuando `cbn_detector` valida un casillero como rechazado, se dispara el pistón automáticamente usando la API de GPIO. Se introdujo una bandera de estado (`piston_fired_for_current_box`) para garantizar un solo disparo por cada caja física observada sin ametrallar el hardware, manejando inteligentemente las transiciones a `nullopt` cuando las cajas salen de la visión.
...
* **El puente a C++ (OpenVINO) también está finalizado**. El modelo `.xml`/`.bin` es procesado a alta velocidad por la lógica de ejecución en `ws_cpp/src/cbn_detector_inference.cpp`.
* **Hardware Integrado:** La actuación física está completada. El GPIO ahora reacciona autónomamente en producción a las deducciones de la inferencia descartando cajas defectuosas mediante actuadores o alarmas. El software está completamente listo para un despliegue y puesta en marcha (*commissioning*) de la línea de producción.
